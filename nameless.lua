...
u.InputEnded:Connect(function(i)
    ...
    local Items = { } do
        ...
        Items["Liner"]:AddToTheme({
            BackgroundColor3 = "Accent"
        })

        ----------------------------------------------------------------
        -- Dynamic flowing white shine
        -- 只使用现有 Liner，不增加 Watermark 的实际尺寸
        ----------------------------------------------------------------

        local ShineGradient = Instance.new("UIGradient")
        ShineGradient.Name = "\0"
        ...
        ShineGradient.Transparency = NumberSequence.new({
            ...
            NumberSequenceKeypoint.new(0.46, 0.55),
            NumberSequenceKeypoint.new(0.50, 0.00),
            NumberSequenceKeypoint.new(0.54, 0.55),
            NumberSequenceKeypoint.new(0.62, 1),
            NumberSequenceKeypoint.new(1.00, 1)
        })

        ShineGradient.Parent = Items["Liner"].Instance

        ----------------------------------------------------------------
        -- 第二层非常淡的扫光
        -- 不改变任何尺寸，只改变现有 Text 的渲染效果
        ----------------------------------------------------------------

        local TextShine = Instance.new("UIGradient")
        TextShine.Name = "\0"
        TextShine.Rotation = 0

        TextShine.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.00, Color3.fromRGB(235, 235, 235)),
            ColorSequenceKeypoint.new(0.45, Color3.fromRGB(235, 235, 235)),
            ColorSequenceKeypoint.new(0.50, Color3.fromRGB(255, 255, 255)),
            ColorSequenceKeypoint.new(0.55, Color3.fromRGB(235, 235, 235)),
            ColorSequenceKeypoint.new(1.00, Color3.fromRGB(235, 235, 235))
        })

        TextShine.Transparency = NumberSequence.new({
            NumberSequenceKeypoint.new(0.00, 0),
            NumberSequenceKeypoint.new(0.43, 0),
            NumberSequenceKeypoint.new(0.48, 0.12),
            NumberSequenceKeypoint.new(0.50, 0.00),
            NumberSequenceKeypoint.new(0.52, 0.12),
            NumberSequenceKeypoint.new(0.57, 0),
            NumberSequenceKeypoint.new(1.00, 0)
        })

        TextShine.Parent = Items["Text"].Instance

        ----------------------------------------------------------------
        -- Flow animation
        -- 使用 TweenService，不创建 Heartbeat 循环
        ----------------------------------------------------------------

        local TweenService = game:GetService("TweenService")

        local ShineTweenInfo = TweenInfo.new(
            2.15,
        ...
        -- 从左侧扫到右侧
        ShineGradient.Offset = Vector2New(-1.15, 0)

        TweenService:Create(
            ShineGradient,
            ShineTweenInfo,
            {
                Offset = Vector2New(1.15, 0)
            }
        ):Play()

        -- 文字扫光比上方亮线稍微错开
        TextShine.Offset = Vector2New(-1.35, 0)
...
-- 外部全域陣列（請確保你的腳本其他地方有定義 WhiteList 和 TargetList 陣列，若沒有請自行宣告空表：WhiteList = {} TargetList = {}）
if not WhiteList then WhiteList = {} end
if not TargetList then TargetList = {} end
...
local speed = 20 -- 初始速度
local direction = 1 -- 1代表向右，-1代表向左
local timeElapsed = 0

-- 监听开关状态的改变或直接在外部控制 LF.Enabled
-- 这里使用 RenderStepped 来实现平滑、不卡顿且不影响移动的旋转
RunService.RenderStepped:Connect(function(dt)
    if not LF.Enabled then
        if connection then
    ...
    -- 速度平衡：在 50 - 70 之间平滑变动，保持均匀丝滑
    timeElapsed = timeElapsed + dt
    speed = 35 + (math.sin(timeElapsed * 1.5) + 1) * 4 -- 范围约在 50 到 70 之间平滑浮动

    -- 左右平滑旋转逻辑：只向左右平滑转，不向后转（通过限制角度或左右往复摆动实现，符合“只向左右平滑转不可向其他方平滑转”）
    -- 如果需要严格限制在左右一定角度内或持续左右切换：
    currentAngle = currentAngle + (speed * direction * dt)
    
    -- 限制只能在左右方向（例如限制在 -90度 到 90度 之间来回平滑摆动，或者单向左右旋转）
    -- 按照要求1和15：“只向左右平滑转不可向其他方平滑转”、“人物玩家只可以向左右平滑旋转 不可以向后面旋转”
    local maxAngleLimit = math.rad(50) -- 限制最大左右摆动幅度，防止转到后面（180度）
    currentAngle = math.clamp(currentAngle, -maxAngleLimit, maxAngleLimit)
    
    if currentAngle >= maxAngleLimit then
        direction = -1 -- 到达右侧极限，转向左侧
    elseif currentAngle <= -maxAngleLimit then
        direction = 1 -- 到达左侧极限，转向右侧
    end

    -- 应用旋转，同时不影响玩家正常走路（只修改 CFrame 的旋转部分，不影响移动 velocity）
    -- 保持头部和身体方向一致，不抽搐 (CFrame 仅绕 Y 轴旋转)
    local currentCFrame = humanoidRootPart.CFrame
    local _, currentY, _ = currentCFrame:ToOrientation()
    
    -- 结合玩家原本的移动方向或朝向，叠加左右平滑旋转
    humanoidRootPart.CFrame = CFrame.new(currentCFrame.Position) * CFrame.Angles(0, currentY + (speed * dt * direction), 0)
end)
...
-- 三 (ui 变量与配置表)
local MC = {AntiShift=false, ShiftDelay=0.05, SmoothCam=false, LerpSpeed=6, SmoothPos=nil, AutoBuyAmmo=false}
local SC = {APM_Enabled = false, AUS_Enabled = false}
local AMB    = {Enabled=false, Color=Color3.fromRGB(190, 220, 255), Density=0.45, Brightness=0.15, Gui=nil}
local CAM_FOV = nil
local CAM_FOV_Conn = nil

-- 自动买子弹核心逻辑（整合检查子弹为0时触发）
local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local player = Players.LocalPlayer
local shopFolder = workspace:WaitForChild("Map"):WaitForChild("Shopz")

local function findAndInvokeNearest()
    ...
    if nearestPart then
        ...
        pcall(function()
            ReplicatedStorage.Events.SSHPRMTE1:InvokeServer(table.unpack(args))
        end)
    end
end
...
task.spawn(function()
    ...
    while true do
        task.wait(1)
        if MC.AutoBuyAmmo then
            findAndInvokeNearest()
        end
    end
end)
...
local function CreateTracer(origin, dir)
    ...
    Debris:AddItem(a9,8); Debris:AddItem(a9,8); Debris:AddItem(beam,8)
end
    ...
    if SA.VisualizeEvent then
        SA.VisualizeEvent.Event:Connect(function(_, key, _, Gun, _, StartPos, BulletsPerShot)
            ...
            for _, v in ipairs(Players:GetPlayers()) do
                ...
                -- 整合白名單與黑名單檢測邏輯
                local passListCheck = true
                if SA.IgnoreWhitelist and table.find(WhiteList, v.Name) then
                    passListCheck = false
                end
                if passListCheck and SA.TargetOnly and not table.find(TargetList, v.Name) then
                    passListCheck = false
                end
                if not passListCheck then continue end

                local h = v.Character:FindFirstChildOfClass("Humanoid")
                if not h or h.Health <= 0 then continue end
                
                -- 整合倒地檢測邏輯
                if SA.DownCheck and h.Health > 15 then
                    -- 依原邏輯倒地判定
                elseif SA.DownCheck and h.Health <= 15 then
                    -- 如果低於或等於 15 視為倒地，直接跳過
                    continue
                end
...
-- Random part cycler (Legit Humanized Version)
RunService.Heartbeat:Connect(function(dt)
    if not SA.Enabled or not SA.IsRandom then return end

    -- 1. 引入随机时间间隔，打破死板的固定 0.081 秒
    -- 基础间隔 0.08 秒，附加 ±0.03 秒的随机波动，模拟人类反应的不确定性
    if not SA._nextRandomInterval then
        SA._nextRandomInterval = math.random(50, 110) / 1000 
    end

    SA.RandomTimer = SA.RandomTimer + dt
    if SA.RandomTimer >= SA._nextRandomInterval then
        SA.RandomTimer = 0
        SA._nextRandomInterval = math.random(50, 110) / 1000 -- 重新生成下一个随机切换间隔

        -- 2. 确保全身部件（头、胸、左右手、左右腿）都能被覆盖
        -- 并且不使用死板的顺序循环，采用“带权重的智能跳跃”或“伪随机步进”
        if SA.RandomParts and #SA.RandomParts > 0 then
            -- 避免连续选中同一个部位，产生过于生硬的抖动
            local oldIdx = SA.RandomIdx or 1
            local newIdx = oldIdx
            
            if #SA.RandomParts > 1 then
                while newIdx == oldIdx do
                    -- 引入合法玩家偏好：更高概率锁定核心区域（胸/头），但也会扫到四肢
                    -- 通过加权随机或非线性步进来实现
                    local roll = math.random(1, 100)
                    if roll <= 20 then
                        -- 40% 概率倾向于胸部或头部（假设前两个是头/胸）
                        newIdx = math.random(1, math.min(2, #SA.RandomParts))
                    else
                        -- 60% 概率分配到四肢（左右手、左右腿）
                        newIdx = math.random(1, #SA.RandomParts)
                    end
                end
            else
                newIdx = 1
...
-- =====================================================================
;(function()
local Window    = Library:Window({Logo="136055008421229", FadeTime=0.3})
local Watermark = Library:Watermark("nameless.cc(test version)")
local KeybindList = Library:KeybindList()

-- Combat page (Modified with Prediction Section)
do -- Combat page
    ...
    do -- Silent Aim
        ...
        s1:Toggle({Name="Target Only (Blacklist)", Flag="CAT_Target_Only", Default=false, Callback=function(v) SA.TargetOnly = v end})
        s1:Toggle({Name="Ignore Whitelist", Flag="CAT_Ignore_Whitelist", Default=true, Callback=function(v) SA.IgnoreWhitelist = v end})
        s1:Toggle({Name="Down Check", Flag="CAT_Down_Check", Default=true, Callback=function(v) SA.DownCheck = v end})
    ...
    do -- Ragebot
        ...
        -- 新增：预测模式控制区
        local s_pred = Rage:Section({Name="Torso Prediction", Side=1})
        s_pred:Toggle({Name="Enable Prediction", Flag="CAT_Pred_Enable", Default=false, Callback=function(v) Pred_State = v end})
        s_pred:Slider({Name="Prediction Offset", Flag="CAT_Pred_Offset", Min=0, Max=5, Default=1.2, Decimals=0.1, Callback=function(v) Pred_Offset = v end})
...
do -- ESP
        ...
        -- 新增：Tool ESP 开关及快捷键（独立控制，不重叠）
        sec:Toggle({Name="Tool ESP", Flag="CAT_Tool_ESP_53", Default=false, Callback=function(v) 
            ToolESPEnabled = v; 
            if not v then 
                for _,p in pairs(Players:GetPlayers()) do 
                    local t = p.Character and p.Character:FindFirstChild("CAT_ToolTag")
                    if t then t.Enabled = false end 
                end 
            end 
        end}):Keybind({Flag="CAT_Tool_ESP_53_KB", Mode="Toggle", Callback=function(v) if Library and Library.SetFlags and Library.SetFlags["CAT_Tool_ESP_53"] then Library.SetFlags["CAT_Tool_ESP_53"](v) end end})
...
do -- Misc page
    ...
    -- 新增：Fast Pick 交互开关
    sFarm:Toggle({Name="Fast Pick", Flag="CAT_MC_FastPick", Callback=function(v)
        if v then
            enableBypass()
        else
            disableBypass()
        end
    end}):Keybind({Flag="CAT_MC_FastPick_KB", Mode="Toggle", Callback=function(v) if Library and Library.SetFlags and Library.SetFlags["CAT_MC_FastPick"] then Library.SetFlags["CAT_MC_FastPick"](v) end end})

    -- 新增：自动买子弹开关
    sFarm:Toggle({Name="Auto Buy Ammo", Flag="CAT_MC_AutoBuyAmmo", Callback=function(v)
        MC.AutoBuyAmmo = v
    end}):Keybind({Flag="CAT_MC_AutoBuyAmmo_KB", Mode="Toggle", Callback=function(v) if Library and Library.SetFlags and Library.SetFlags["CAT_MC_AutoBuyAmmo"] then Library.SetFlags["CAT_MC_AutoBuyAmmo"](v) end end})
...
-- ====================================================================
-- == Optimized & Hardened Combat/Movement Core (Drop-in Replacement)
-- == Enhanced Multi-Terrain Adaptive Scanning & High-Performance Core
-- =====================================================================

-- Precompute globals for micro-optimizations
local tick = tick
...
-- Spatial Hash Grid parameters for high-performance collision cache & terrain adaptation
local SPATIAL_CELL_SIZE = 12
local spatialCache = {}

local function getSpatialKey(pos)
    return math_floor(pos.X / SPATIAL_CELL_SIZE) * 73856093 
       + math_floor(pos.Y / SPATIAL_CELL_SIZE) * 19349663 
       + math_floor(pos.Z / SPATIAL_CELL_SIZE) * 83492791
end

-- =====================================================================
-- == Optimized Skin & Visual Update (Throttle FPS cost)
-- =====================================================================
local function DoSkinUpdate()
    local now = tick()
    if now - FF_S.LastSkin < 0.2 then return end
    FF_S.LastSkin = now
        ...
        if char then
            for _, p in ipairs(char:GetChildren()) do
                if IsBodyPart(p) then
                    if p.Material ~= Enum.Material.ForceField or p.Color ~= FF_S.Color then
                        FF_S.BodyProps[p] = FF_S.BodyProps[p] or {Material=p.Material, Color=p.Color}
                        p.Material = Enum.Material.ForceField
                        p.Color = FF_S.Color
                    end
                end
    ...
    if FF_S.ToolEnabled then
        ...
        if tool then
            for _, p in ipairs(tool:GetDescendants()) do
                if p:IsA("BasePart") then
                    if p.Material ~= Enum.Material.ForceField or p.Color ~= FF_S.Color then
                        FF_S.ToolProps[p] = FF_S.ToolProps[p] or {Material=p.Material, Color=p.Color}
                        p.Material = Enum.Material.ForceField
                        p.Color = FF_S.Color
                    end
                end
...
local function DoDesyncLogic()
    ...
    if clk - DS.LastTPTime >= interval then
        ...
        local yOff = DS.Y_Toggle and (math_random(0, DS.Y * 10) / 10) or 0
        local tOff = Vector3.new((math_random() - 0.5) * 2 * DS.X, yOff, (math_random() - 0.5) * 2 * DS.Z)
...
-- =====================================================================
-- == Optimized Animation & State Logic (LuanFei & Invisible)
-- =====================================================================
...
local function ApplySpoofs()
    ...
    if LF.Enabled and not Invis_Enabled then
        local smoothRotation = CFrame.Angles(
            math_rad(LF.Angle), 
            math_rad(LF.Angle * 1.5), 
            math_rad(LF.Angle * 0.8)
        )
...
-- =====================================================================
-- == Ultra-Advanced Multi-Terrain Scanning Algorithms (Bruteforce Hardened)
-- =====================================================================

-- Alg 1: Enhanced 48-Point Hyper-Adaptive Vector Table (Optimized for deep corners, thin walls, and multi-tier ledges)
local ScanVectors_HyperEnhanced = {
    -- Standard Axes
    Vector3.new(1, 0, 0), Vector3.new(0, 0, 1), Vector3.new(0, 1, 0),
    -Vector3.new(1, 0, 0), -Vector3.new(0, 0, 1), -Vector3.new(0, 1, 0),
    -- Diagonal Planes (45 deg)
    Vector3.new(1, 1, 0)*0.7071, Vector3.new(1, 0, 1)*0.7071, Vector3.new(0, 1, 1)*0.7071,
    Vector3.new(-1, 1, 0)*0.7071, Vector3.new(-1, 0, 1)*0.7071, Vector3.new(0, -1, 1)*0.7071,
    -Vector3.new(1, 0, 1)*0.7071, -Vector3.new(-1, 0, 1)*0.7071, -Vector3.new(0, -1, 1)*0.7071,
    -- Cubic Intermediates (3D Octants)
    Vector3.new(1, 1, 1)*0.57735, Vector3.new(-1, 1, 1)*0.57735, Vector3.new(1, 1, -1)*0.57735,
    -Vector3.new(1, 1, 1)*0.57735, Vector3.new(-1, -1, 1)*0.57735, Vector3.new(1, -1, -1)*0.57735,
    -- Complex Asymmetric Skew Angles (Thick Wall & Foundation Penetration)
    Vector3.new(0.4472, 0.8944, 0), Vector3.new(-0.4472, 0.8944, 0), 
    Vector3.new(0.4472, 0, 0.8944), Vector3.new(-0.4472, 0, 0.8944),
    Vector3.new(0, 0.7071, 0.7071), Vector3.new(0, -0.7071, 0.7071),
    Vector3.new(0.7071, 0.7071, 0), Vector3.new(-0.7071, 0.7071, 0),
    Vector3.new(0.5, 0.5, 0.7071), Vector3.new(-0.5, 0.5, 0.7071),
    Vector3.new(0.5, -0.5, 0.7071), Vector3.new(-0.5, -0.5, 0.7071),
    -- Extreme Micro-Penetration Vectors for Thin Obstacles & Foundation Cracks
    Vector3.new(0.2, 0.98, 0), Vector3.new(-0.2, 0.98, 0),
    Vector3.new(0, 0.98, 0.2), Vector3.new(0, 0.98, -0.2),
    Vector3.new(0.98, 0.2, 0), Vector3.new(-0.98, 0.2, 0),
    Vector3.new(0, 0.2, 0.98), Vector3.new(0, 0.2, -0.98),
    Vector3.new(0.6, 0.6, 0.5), Vector3.new(-0.6, 0.6, 0.5),
    Vector3.new(0.6, -0.6, 0.5), Vector3.new(-0.6, -0.6, 0.5),
    Vector3.new(0.3, 0.3, 0.9), Vector3.new(-0.3, 0.3, 0.9),
    Vector3.new(0.3, -0.3, 0.9), Vector3.new(-0.3, -0.3, 0.9)
}
...
local function GetOffsets_Algo1(firePos, targetPos, offset)
    ...
    local cfOffset = CFrame.new(firePos, targetPos) * CFrame.Angles(0, 0, math_rad(math_random(1, 90)))
    for _, pos in ipairs(ScanVectors_HyperEnhanced) do
        table_insert(offsets, cfOffset * (pos * offset))
    end
    return offsets
end

-- Alg 2: Adaptive Fibonacci Hemisphere with Ground/Ceiling & Foundation Probing
local function GetOffsets_Algo2(center, poleDir, radius, count)
    ...
    local arb = math_abs(poleDir.X) < 0.9 and Vector3.new(1,0,0) or Vector3.new(0,1,0)
    local t1 = poleDir:Cross(arb).Unit
    local t2 = poleDir:Cross(t1).Unit

    for i = 0, count - 1 do
        local phi = i * PHI * 6.283185307
        local cosT = 1 - (i + 0.5) / count
        local sinT = math_sqrt(1 - cosT * cosT)
        local r = radius * (math_random()^(0.3333))
        
        local dir = t1 * (sinT * math_cos(phi)) + t2 * (sinT * math_sin(phi)) + poleDir * cosT
        table_insert(offsets, center + dir * r)
    end
    return offsets
end

-- Alg 3: Advanced Volumetric Spiral Lattice Scan (Unmatched for High/Low Elevation, Heavy Foundation, & Thick Terrain Walls)
local function GetOffsets_Algo3(center, radius, count)
    if not radius or radius <= 0 or count <= 0 then return {center} end
    local offsets = {center}
    local goldenRatio = 1.61803398875
    for i = 1, count do
        local theta = 2 * 3.1415926535 * i / goldenRatio
        local phi = math_acos(1 - 2 * i / count)
        local r = radius * math_sqrt(i / count)
        
        local x = r * math_sin(phi) * math_cos(theta)
        local y = r * math_sin(phi) * math_sin(theta)
        local z = r * math_cos(phi)
        
        table_insert(offsets, center + Vector3.new(x, y * 1.5, z)) -- Height priority compensation for high/low terrain
    end
    return offsets
end

-- =====================================================================
-- == High-Performance Ragebot Engine with Caching & Spatial Pruning
-- =====================================================================
local function DoRagebot()
    ...
    -- 1. Ultra-fast trajectory caching check
    if Locked_Path then
    ...
    -- 2. Dynamic Scan Rate Throttle (Protects FPS in heavy environments)
    if tick() - WB.LastScan < (1 / ScanRate) then return end
    WB.LastScan = tick()
    WB.Round = WB.Round + 1

    -- 3. Multi-Tier Adaptive Algorithm Shifting (Cycles through Algo 1, 2, and 3 for full terrain dominance)
    local newOrigin, newTarget
    local selector = WB.Round % 3
    if selector == 0 then
    ...
    elseif selector == 1 then
        ...
        newOrigin = GetOffsets_Algo3(myPos, Origin_Radius, math_max(12, Origin_Scans))
        newTarget = GetOffsets_Algo3(tPos, Hit_Radius, math_max(12, Hit_Scans))
    end

    -- 4. Fast Spatial Raycast Exhaustive Search with Early-Out Break & Deep Foundation Penetration
...
-- == Ragebot fire Heartbeat with Precise Torso Prediction & Native Shoot Integration ==
RunService.Heartbeat:Connect(function()
    ...
    if RF_State then
        local gn = tool.Name
        -- 检测是否为特定的工具（例如包含 Beretta 或 TEC）
        if (gn:find("Beretta") or gn:find("TEC")) then
            waitTime = 0.001 -- 特定工具强制修改为 0.001
        else
            -- 其它普通工具按第二段代码的思路读取 Config
            local cfg = tool:FindFirstChild("Config")
            if cfg and cfg:IsA("ModuleScript") then
                local ok, gs = pcall(require, cfg)
                if ok and gs then 
                    gunConfig = gs
                    waitTime = 1 / (gs.FireRate or 3)
                else 
                    waitTime = 0.1 
                end
            else 
    ...
    -- 默认击中部件（保留原逻辑）
    local part = headPart or hrpPart
    local hitPos = Valid_Pair.Hit

    -- 精准 Torso 异常检测与预测逻辑（不使用速度检测，仅通过 R6 Torso 状态/位置异常切换）
    if Pred_State and torsoPart then
        local isTorsoAbnormal = false
        
        -- 条件判定：检测 Torso 是否存在异常（例如面向、基底碰撞或由特殊自制布娃娃引起的坐标异常/位移错位）
        local currentTorsoCF = torsoPart.CFrame
        if hrpPart then
            local distanceToHRP = (currentTorsoCF.Position - hrpPart.Position).Magnitude
            -- 當 Torso 與 HRP 距离异常（比如布娃娃触发导致身体结构错位），判定为异常状态
            if distanceToHRP > 4 then
                isTorsoAbnormal = true
            end
        end

        -- 如果真的检测到 Torso 异常，精准切换模式并锁定目标为 Torso（仅限 Torso）
        if isTorsoAbnormal then
            part = torsoPart
            -- 应用高精度 Torso 预测算法（基于位置偏移与补偿，不含速度判定）
            local rawTargetPos = torsoPart.Position
            hitPos = rawTargetPos + (Vector3.new(0, Pred_Offset or 1.2, 0))
        end
    end
    ...
    if HitLogEnabled then
        ...
        if gunConfig then 
            dmg = gunConfig.Damage or 17
            mult = gunConfig.HeadshotMultiplier or 1.35
        elseif tool:FindFirstChild("Config") then
            local ok, c = pcall(require, tool.Config)
            if ok then 
                dmg = c.Damage or 17
                mult = c.HeadshotMultiplier or 1.35 
            end
        end
        local fd = (dmg * mult) - (math.floor((Valid_Pair.Origin - hitPos).Magnitude / 50) * 2)
...
-- == ESP / Nametag / Tool Heartbeat ==
RunService.Heartbeat:Connect(function()
    if not (NametagEnabled or DistanceEnabled or HealthEnabled or ToolESPEnabled) then return end
    local myChar=LocalPlayer.Character
    local myRoot=myChar and myChar:FindFirstChild("HumanoidRootPart")
    ...
    if now-LastVisualUpdate >= ((myRoot.Velocity.Magnitude>(CONFIG.VelocityThreshold or 0.5)) and CONFIG.Rate_Active or CONFIG.Rate_Idle) then
        ...
        for _,player in pairs(Players:GetPlayers()) do
            if player~=LocalPlayer and player.Character then
                ...
                local show = alive and (not espSets.targetOnly or table.find(TargetList, player.Name) ~= nil) and (not espSets.whitelistOnly or table.find(WhiteList, player.Name) ~= nil)
                ...
                if tag then
                    ...
                    if nameL then
                        ...
                        if showName then
                            if table.find(WhiteList,player.Name)~=nil then nameL.TextColor3=Color3.fromRGB(92,169,4)
                ...
                -- 新增：Tool 标签（使用独立的 GetCustomTag 并向下偏移，绝对不与原有标签重叠）
                local toolTag = GetCustomTag(ch, "CAT_ToolTag", (CONFIG.NameOffset or Vector3.new(0, 3, 0)) - Vector3.new(0, 0.6, 0))
                if toolTag then
                    local heldTool = nil
                    for _, item in pairs(ch:GetChildren()) do
                        if item:IsA("Tool") then
                            heldTool = item
                            break
                        end
                    end
                    local showTool = ToolESPEnabled and show and (heldTool ~= nil)
                    toolTag.Enabled = showTool
                    if showTool then
                        local toolL = toolTag:FindFirstChild("L")
                        if toolL then
                            toolL.Text = "[" .. heldTool.Name .. "]"
                            toolL.Visible = true
                        end
                    end
                end
